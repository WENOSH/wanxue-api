...
/* Guided Quiz */
.guided-hint { margin-top:10px; padding:12px; background:#fff8e7; border:1.5px solid var(--accent); border-radius:10px; font-size:15px; }
.guided-retry { display:inline-block; margin-top:8px; margin-right:8px; padding:8px 18px; border:none; border-radius:20px; font-size:14px; cursor:pointer; font-family:inherit; }
.guided-retry.retry { background:var(--secondary); color:white; }
.guided-retry.reveal { background:#ddd; color:#555; }
.guided-followup { margin-top:15px; padding:14px; background:#f0fdfa; border:2px solid var(--secondary); border-radius:12px; font-size:15px; }
.guided-followup textarea { width:100%; min-height:60px; border:1.5px solid #ddd; border-radius:8px; padding:8px; font-size:14px; font-family:inherit; margin-top:8px; resize:vertical; }
/* TTS 引擎选择器 */
.tts-engine-selector { flex-shrink:0; display:flex; align-items:center; gap:4px; }
.tts-engine-btn { width:32px; height:32px; border:none; border-radius:50%; background:rgba(255,255,255,0.7); color:var(--text); font-size:12px; font-weight:700; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all 0.2s; font-family:inherit; }
.tts-engine-btn:hover { background:rgba(255,255,255,0.95); }
.tts-engine-btn:active { transform:scale(0.9); }
.tts-engine-btn.active { background:var(--secondary); color:white; }
"""
...
function speakCard(btn) {
  ...
  // 如果当前正在朗读，停止
  if (_ttsEngine === 'web' && window.speechSynthesis && window.speechSynthesis.speaking) {
...
function speakWithEdge(text, btn) {
  btn.classList.add('speaking');
  // 使用 Edge TTS 免费 API 代理
  var url = 'https://speech.platform.bing.com/consumer/speech/synthesize/readaloud/edge/v1';
  var ssml = '<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="zh-CN">'
    + '<voice name="zh-CN-XiaoxiaoNeural">'
    + text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;')
    + '</voice></speak>';

  // 尝试通过 fetch 调用 Edge TTS API
  fetch('https://southeastasia.tts.speech.microsoft.com/cognitiveservices/v1', {
    method: 'POST',
    headers: {
    ...
    if (!r.ok) throw new Error('Edge TTS 不可用');
    return r.blob();
  }).then(function(blob) {
...
function fallbackWebTTS(text, btn) {
  if (window.speechSynthesis) {
    var utterance = new SpeechSynthesisUtterance(text);
    utterance.lang = 'zh-CN';
    utterance.rate = 0.95;
...
function toggleTTSEngine(btn) {
  if (_ttsEngine === 'web') {
    ...
    btn.title = '当前引擎: Edge TTS';
    localStorage.setItem(STORAGE_KEY + '_tts', 'edge');
  } else {
    _ttsEngine = 'web';
    btn.textContent = 'Web';
    btn.classList.remove('active');
    btn.title = '当前引擎: Web Speech';
    localStorage.setItem(STORAGE_KEY + '_tts', 'web');
  }
}
...
function checkAnswer(btn, isCorrect, fbId) {
  ...
  // 检查引导模式
  var guidedStr = card ? card.getAttribute('data-guided') : null;
  var guided = null;
  if (guidedStr) { try { guided = JSON.parse(guidedStr); } catch(e) {} }

  var btns = box.querySelectorAll('.answer-btn');
  var btnIdx = -1;
  for (var i = 0; i < btns.length; i++) { if (btns[i] === btn) { btnIdx = i; break; } }

  if (guided && guided.enable) {
    // ===== 引导模式 =====
    // 清理旧的控制按钮区域
    var oldActions = box.querySelector('.guided-action-area');
    ...
    if (isCorrect) {
      ...
      var attempts = parseInt(card.getAttribute('data-guided-attempts') || '0');
      if (attempts === 0) {
        fb.innerHTML = '🎉 <strong>答对了！</strong>';
        if (guided.correct_followup) {
          fb.innerHTML += '<div class="guided-followup"><strong>追问：</strong>' + guided.correct_followup + '<textarea placeholder="说说你的想法..." onfocus="this.style.borderColor=\'#4ecdc4\'"></textarea></div>';
        }
      } else {
        fb.innerHTML = '🎉 <strong>很棒！</strong>答对了！';
        // 显示完整解析
        var explainHtml = '<div style="font-weight:600;margin-bottom:6px;color:#2d3047;margin-top:10px">📖 解析：</div>';
        for (var i = 0; i < btns.length; i++) {
          var label = String.fromCharCode(65 + i);
          var explain = btns[i].getAttribute('data-explain') || '';
          var correctMark = '';
          for (var j = 0; j < btns.length; j++) {
            if (btns[j].getAttribute('onclick').indexOf(',true,') > 0 && j === i) correctMark = '✅';
          }
          explainHtml += '<div style="margin:4px 0;color:#2d3047"><b>' + correctMark + ' ' + label + '.</b> ' + btns[i].textContent.trim();
          if (explain) explainHtml += ' <span style="color:#6c6f7d">— ' + explain + '</span>';
          explainHtml += '</div>';
        }
        fb.innerHTML += '<div style="margin-top:10px;padding:10px;background:#f8f5f0;border-radius:10px;font-size:13px;line-height:1.6;">' + explainHtml + '</div>';
      }
      card.removeAttribute('data-guided-attempts');
    } else {
      // 答错
      btn.classList.add('wrong');
      var hint = (guided.wrong_hints && guided.wrong_hints['btn_' + btnIdx]) || '再想想～';
      fb.className = 'feedback show bad';
      fb.innerHTML = '🤔 ' + hint;

      var attempts = parseInt(card.getAttribute('data-guided-attempts') || '0');
      attempts++;
      card.setAttribute('data-guided-attempts', attempts);

      // 控制按钮区
      var actionsDiv = document.createElement('div');
      actionsDiv.className = 'guided-action-area';
      actionsDiv.style.cssText = 'margin-top:8px;';
      if (attempts < guided.max_retries) {
        var retryBtn = document.createElement('button');
        retryBtn.className = 'guided-retry retry';
        retryBtn.textContent = '再试一次';
        retryBtn.onclick = function() { guidedRetry(box); };
        actionsDiv.appendChild(retryBtn);
      }
      var revealBtn = document.createElement('button');
      revealBtn.className = 'guided-retry reveal';
      revealBtn.textContent = '直接看解析';
      revealBtn.onclick = function() { guidedReveal(box); };
...
function guidedReveal(box) {
  ...
  for (var i = 0; i < btns.length; i++) {
    var label = String.fromCharCode(65 + i);
    var explain = btns[i].getAttribute('data-explain') || '';
    var isCorrect = btns[i].getAttribute('onclick').indexOf(',true,') > 0;
    if (isCorrect) btns[i].classList.add('correct');
    explainHtml += '<div style="margin:4px 0;color:#2d3047"><b>' + (isCorrect ? '✅ ' : '') + label + '.</b> ' + btns[i].textContent.trim();
    if (explain) explainHtml += ' <span style="color:#6c6f7d">— ' + explain + '</span>';
    explainHtml += '</div>';
  }

  fb.className = 'feedback show bad';
  fb.innerHTML = '📖 <strong>解析：</strong><br>' + explainHtml + '<div style="margin-top:10px;text-align:right"><button class="guided-retry reveal" onclick="guidedDismiss(this)">知道了</button></div>';

  if (card) card.removeAttribute('data-guided-attempts');
}

function guidedDismiss(btn) {
  var box = btn.closest('.game-box');
  var fb = box.querySelector('.feedback');
  if (fb) {
    fb.className = 'feedback';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // 恢复 TTS 引擎设置
  var savedEngine = localStorage.getItem(STORAGE_KEY + '_tts');
  var ttsBtn = document.getElementById('ttsToggle');
  if (savedEngine === 'edge') {
    _ttsEngine = 'edge';
    if (ttsBtn) { ttsBtn.textContent = 'Edge'; ttsBtn.classList.add('active'); ttsBtn.title = '当前引擎: Edge TTS'; }
  } else {
    _ttsEngine = 'web';
    if (ttsBtn) { ttsBtn.textContent = 'Web'; ttsBtn.title = '当前引擎: Web Speech'; }
  }
  if (ttsBtn) {
    ttsBtn.onclick = function() { toggleTTSEngine(this); };
  }
  // 首次渲染（在覆盖之后，确保按钮文字正确）
  updateUI();
});
...
<div class="top-bar">
  <div class="top-bar-row">
    <h1>{title_safe}</h1>
    <button class="tts-engine-btn" id="ttsToggle" title="TTS 引擎切换">Web</button>
...
def _build_cards_html(chapters: list) -> str:
    ...
    for ch_idx, ch in enumerate(chapters):
        ...
        for card_idx, card in enumerate(cards):
            ...
            # 引导式问答数据
            guided_data = card.get("_guided")
            guided_attr = ""
            if guided_data:
                guided_json = json.dumps(guided_data, ensure_ascii=False)
                guided_attr = f' data-guided=\'{guided_json}\''
            if is_last_reward:
                diff_feedback_html = """
...
"""
            ...
            cards_html.append(
                ...
                f'data-ch="{ch_id}" data-idx="{card_idx}" data-type="{card_type}"{guided_attr}>\n'
